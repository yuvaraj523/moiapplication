...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateSql = "UPDATE addmoi SET date='$date', the_groom='$the_groom', the_bride='$the_bride', marriage_type='$marriage_type', name='$name', address='$address', mobile='$mobile', amount='$amount' WHERE id='$id'";

    if ($con->query($updateSql) === TRUE) {
        $_SESSION['success_message'] = 'Record updated successfully!';
        header("Location: addmoidisplay.php?message=update_success");
        exit;
    } else {
...
<div class="form-container">
        <h2> Edit MOI Details</h2>
        <form id="marriageForm" action="addmoi_edit.php?id=<?php echo htmlspecialchars($id); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" id="the_groom" name="the_groom" value="<?php echo htmlspecialchars($the_groom); ?>" required>
            </div>
            
            <div class="form-group">
                <label for="the_bride"><i class="fa fa-female" aria-hidden="true"></i> Bride (Name)</label>
                <input type="text" id="the_bride" name="the_bride" value="<?php echo htmlspecialchars($the_bride); ?>" required>
            </div>
            
            <div class="form-group">
                <label><i class="fa fa-check" aria-hidden="true"></i> Marriage Type</label>
                <input type="checkbox" id="marriage-boy" name="marriage_type[]" value="boy" <?php echo in_array('boy', $marriage_type) ? 'checked' : ''; ?>>
                <label for="marriage-boy">The Groom</label>
                <input type="checkbox" id="marriage-girl" name="marriage_type[]" value="girl" <?php echo in_array('girl', $marriage_type) ? 'checked' : ''; ?>>
                <label for="marriage-girl">The Bride</label>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="tel" id="mobile" name="mobile" value="<?php echo htmlspecialchars($mobile); ?>" placeholder="Please enter a 10-digit mobile number" pattern="\d{10}" maxlength="10" required>
            </div>
            
            <div class="form-group">
                <label for="amount"><i class="fa fa-inr" aria-hidden="true"></i>Amount</label>
                <input type="number" id="amount" name="amount" value="<?php echo htmlspecialchars($amount); ?>" placeholder="Enter the amount" required>
            </div>
...
<script type="text/javascript" src="//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit"></script>
 <script>
        ...
        const marriageBoyCheckbox = document.getElementById('marriage-boy');
        const marriageGirlCheckbox = document.getElementById('marriage-girl');
    
        function handleCheckboxChange(checkedCheckbox) {
            if (checkedCheckbox === marriageBoyCheckbox) {
                marriageGirlCheckbox.checked = false;
            } else if (checkedCheckbox === marriageGirlCheckbox) {
                marriageBoyCheckbox.checked = false;
            }
        }
    
        marriageBoyCheckbox.addEventListener('change', function() {
            if (marriageBoyCheckbox.checked) {
                handleCheckboxChange(marriageBoyCheckbox);
            }
        });
    
        marriageGirlCheckbox.addEventListener('change', function() {
            if (marriageGirlCheckbox.checked) {
                handleCheckboxChange(marriageGirlCheckbox);
            }
        });
